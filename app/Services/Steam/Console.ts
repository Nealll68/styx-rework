...
class SteamConsole { 
  readonly outputRegex = /Update state \((0x\d+)\) (\w+), progress: (\d+.\d+) \((\d+) \/ (\d+)\)$/
  ...
  public async updateArma (steamAccount?: SteamAccountInterface): Promise<void> {
    ...
    if (!account.username || !account.password) throw new NoSteamAccountException()
    if (!account.guard && steamConfig.guard) throw new SteamGuardRequiredException()
    ...
    try {
      for await (const line of this._steamCmd.run(commands, { noAutoLogin: true })) {
